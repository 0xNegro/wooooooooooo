      ...
      if (token.isSheep) {
        if (claims[i].unstaked) {
          if (claims[i].earned.eq(BigNumber.from(0))) {
            ...
            o.push({
              message: `Sheep #${token.number} left the barn and evaded the Wolves, earning ${parseBigNumber(claims[i].earned)} $WOOL`,
              source: './images/unstaked-safe.gif'
            })
          }
        } else {
          o.push({
            message: `Sheep #${token.number} was sheared for ${parseBigNumber(claims[i].earned)} $WOOL, after paying a 20% tax to the Wolves.`,
        ...
        if (claims[i].unstaked) {
          o.push({
            message: `Wolf #${token.number} left the pack, and received ${parseBigNumber(claims[i].earned)} $WOOL!`,
            source: './images/unstaked-pack.gif'
          })
        } else {
          o.push({
            message: `Wolf #${token.number} collected a tax of ${parseBigNumber(claims[i].earned)} $WOOL!`,
    ...
    <Container>
      <div className="flex flex-col items-center font-pixel gap-5" >
        ...
        <EthereumInteraction wallet={wallet} chain={chain}>
          {fetching ? (
            <div className="text-center text-red font-console">
              Fetching...
            </div>
          ) : (
            <TokenList title={'Can Stake'} active={operation !== 'CLAIM'} items={tokens} selected={selected} toggleSelected={(item, select) => {
        ...
        <EthereumInteraction wallet={wallet} chain={chain}>
          {fetching ? (
            ...
            <div className="w-full flex flex-col justify-center items-center gap-2">
              <TokenList title={'BARN'} active={operation !== 'STAKE'} items={stakes?.filter((el) => el.isSheep)} selected={selected} toggleSelected={(item, select) => {
              ...
              <TokenList title={'Wolfpack'} active={operation !== 'STAKE'} items={stakes?.filter((el) => !el.isSheep)} selected={selected} toggleSelected={(item, select) => {
                if (operation !== 'CLAIM' && operation !== null) return
                setOperation('CLAIM')
                setSelected((current) => {
                  return select ? current.concat(item) : current.filter((el) => el.id !== item.id)
              ...
              <div className="w-full flex flex-col md:flex-row justify-center items-center gap-1">
                ...
                {operation === 'CLAIM' && (
                  <>
                    <WoodButton width={150} height={80} fontSize="16px" title={'SHEAR $WOOL'} loading={loading} onClick={() => {
                      const isClaimingSheep = !!selected.find((el) => el.isSheep)
                      const isClaimingWolf = !!selected.find((el) => !el.isSheep)
                      const scenes = []
                      if (isClaimingSheep) scenes.push({message:'Shearing sheep', source:'./images/shearing.gif'})
                      if (isClaimingWolf) scenes.push({message:'Collecting 20% tax', source:'./images/claiming-pack.gif'})
                      setLoadingScenes(scenes)
                      onClaim(false)
                    }}/>
                    <div className={canUnstake() ? '' : 'opacity-50'}>
                    <WoodButton width={150} height={80} fontSize="16px" title={'SHEAR $WOOL AND UNSTAKE'} disabled={!canUnstake()} loading={loading} onClick={() => {
                      ...
                      const scenes = []
                      if (isUnstakingSheep) scenes.push({message:'Shearing and leaving the barn', source:'./images/unstaking-barn.gif'})
                      if (isUnstakingWolf) scenes.push({message:'Collecting tax and leaving the pack', source:'./images/unstaking-pack.gif'})
                      setLoadingScenes(scenes)
                      onClaim(true)
                    }}/>
                    </div>
                ...
                {operation === 'STAKE' && (
                  <WoodButton width={150} height={80} fontSize="16px" title={'STAKE'} loading={loading} onClick={() => {
                    const isStakingSheep = !!selected.find((el) => el.isSheep)
                    const isStakingWolf = !!selected.find((el) => !el.isSheep)
                    const scenes = []
                    if (isStakingSheep) scenes.push({message:'Entering the Barn', source:'./images/staking-barn.gif'})
                    if (isStakingWolf) scenes.push({message:'Joining the Wolfpack', source:'./images/staking-pack.gif'})
                    setLoadingScenes(scenes)
                    onStake(selected)
                  }}/>
                )}
              ...
              {operation === 'CLAIM' && !canUnstake() && (
                <div className="text-xs text-center text-red" style={{width:'300px'}}>
                  You can only unstake a Sheep if it has at least 2 days worth of $WOOL.
                </div>
              )}
            </div>
          )}
      ...
      {error && (
        <div className="text-sm text-red font-console">
          {error}
        </div>
      )}
      <LoadingModal loadingScenes={loadingScenes} modalIsOpen={transacting} />
      <OutcomeModal outcomes={outcomes} modalIsOpen={outcomes.length > 0} closeModal={() => {
        reload()
        setOutcomes([])
        setSelected([])
      }}/>
      <UnstakeModal modalIsOpen={isUnstaking} closeModal={() => {setIsUnstaking(false)}} loading={loading} onClick={() => {
        const isUnstakingSheep = !!selected.find((el) => el.isSheep)
        const isUnstakingWolf = !!selected.find((el) => !el.isSheep)
        const scenes = []
        if (isUnstakingSheep) scenes.push({message:'Shearing and leaving the Barn', source:'./images/unstaking-barn.gif'})
        if (isUnstakingWolf) scenes.push({message:'Collecting tax and leaving the Wolfpack', source:'./images/unstaking-pack.gif'})
        setLoadingScenes(scenes)
        onClaim(true)
      }}/>
    </Container>
